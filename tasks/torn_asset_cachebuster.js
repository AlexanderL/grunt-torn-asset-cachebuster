...
function cacheBustCss(css, options) {
    var img = options.cssRegex;
    return css.replace(img, replace('url({p1}?v={buster})', options));
}

function cacheBustHtml(html, options) {

    html = html.replace(options.cleanCachebustRegexp, '$1');

    var css = options.cssInHtmlRegexp;
    html = html.replace(css, replace('{p1}?v={buster}', options));

    var js = options.jsInHtmlRegexp;
    html = html.replace(js, replace('{p1}?v={buster}', options));

    var images = options.imgInHtmlRegexp;
    html = html.replace(images, replace('{p1}?v={buster}', options));
...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('torn_asset_cachebuster', 'Cachebust images, scripts and other assets in your HTML & CSS files.', function() {
        ...
        var options = this.options({
            cssRegex: /url\(['"]?(?!data:)([^)'"?]+)['"]?(?:\?v=[0-9]+)*\)/gi,
            cleanCachebustRegexp:/(\.js|\.css|\.(?:png|gif|jpg|jpeg))(\?v=[0-9.]+)+/gi,
            cssInHtmlRegexp: /((getVersionStatic\(|<link\s.+[href=].+)["'\']+(.+\.css(?:\?v=[0-9]+)?))+/gi,
            jsInHtmlRegexp: /((getVersionStatic\(|<script\s.+[src=].+)["'\']+(.+\.js(?:\?v=[0-9]+)?))+/gi,
            imgInHtmlRegexp: /(<img\s.+[src=].+["'\']+(.+\.(?:png|gif|jpg|jpeg)(?:\?v=[0-9]+)?))+/gi,
            buster: '123456',
            ignore: [],
            htmlExtension: 'html'
        });
